 ...
 * グループ名設定コマンド（正常系）
 */
func TestCommandAnalyzeSetNameNormally(t *testing.T) {
	opt := aetest.Options{StronglyConsistentDatastore: true} //データストアに即反映
	instance, err := aetest.NewInstance(&opt)
	if err != nil {
		t.Fatalf("Failed to create aetest instance: %v", err)
	...
	//送信メッセージの検証
	if !regexp.MustCompile("グループ（もしくはトークルーム）の名前を設定しました").MatchString(actualSendMessages[0]) {
		t.Errorf("Unmatch send message text: %v", actualSendMessages[0])
	}

	// データストアにDisplayNameが書き込まれていること
	var actualEntity subscriber
	if err = datastore.Get(ctx, key, &actualEntity); err != nil {
		t.Fatal(err)
	}
	if actualEntity.DisplayName != expectedName {
		t.Errorf("Unmatch entitiy's name. name='%v'", actualEntity.DisplayName)
	}
}
